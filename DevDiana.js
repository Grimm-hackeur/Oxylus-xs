...
function inconnuboyLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [OXYLUS-XMD] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363421117788420@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© XENIX OFFICIAL',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:DIANA TECH\nORG:DIANA TECH;\nTEL;type=CELL;type=VOICE;waid=50947118426:50947118426\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© XENIXZ' } }
                    }},
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'OXYLUS XS is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 OXYLUS XS — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
